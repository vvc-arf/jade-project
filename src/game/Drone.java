...
public class Drone extends Agent {
    ...
    private AID[] commonMilitaryAgents;
    private final ArrayList<AID> ownMilitaryAgents = new ArrayList<>();

    @Override
    protected void setup() {
        System.out.printf("Агент-дрон %s запущен.%n", getAID().getName());
        ...
        if (args != null && args.length > 0) {
            ...
            addBehaviour(new SequenceMoveServer());
        } else {
            // Уничтожаем агента
    ...
    protected void takeDown() {
        System.out.printf("Агент-дрон %s уничтожен.%n", getAID().getName());
    }
